...
// PruneUnapproved removes packages that no longer have an active approval.
// This ensures removing an entry from approved-plugins.json also removes it
// from the published catalog during the next catalog update.
func PruneUnapproved(index RepositoryIndex, registry ApprovalRegistry) RepositoryIndex {
	filtered := make([]CatalogPackage, 0, len(index.Plugins))
	for _, pkg := range index.Plugins {
		if pkg.Manifest == nil {
			continue
		}
		repository := strings.TrimPrefix(pkg.RepoURL, "https://github.com/")
		approval, err := registry.Resolve(repository, pkg.Manifest.GetPluginId())
		if err != nil {
			continue
		}
	...
	sort.Slice(index.Plugins, func(i, j int) bool {
		return packagePluginID(index.Plugins[i]) < packagePluginID(index.Plugins[j])
	})
	return index
}
